...
> Full-screen live WebGL shader wallpaper.
...
2. Double-click `install.command` (or run `./install.sh` in Terminal).
   It copies `animated-wallpaper.widget` into your Übersicht widgets folder, installs any
   helpers, and walks you through any configuration.

The installer is safe to re-run; it just refreshes the install in place.
To install by hand instead, unzip `animated-wallpaper.widget.zip` into
